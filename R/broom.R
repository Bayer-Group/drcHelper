...
#' @export
broom_williams <- function(
  x,
  method = c("Williams_PMCMRplus", "Williams_JG"),
  ...
) {
    ...
    williams_result <- try(
      {
        if (inherits(x, "formula")) {
          ...
          stop(
            "x must be a formula, aov, or lm object for Dunnett_multcomp method"
          )
        }
        ...
        comparisons <- paste(
          names(summary_result$test$coefficients),
          ifelse(wt$alternative == "greater", " <= 0", " >= 0")
        )
    ...
    williams_result <- try(
      {
        ...
        if (
          is.null(list(...)$direction) || list(...)$direction == "decreasing"
        ) {
        ...
        # --- END PATCH ---
        wt <- drcHelper::williamsTest_JG(
          df = model_data,
          resp = resp_name,
          trt = clean_factor_name,
          direction = direction
        )
        if (
          is.null(list(...)$direction) || list(...)$direction == "decreasing"
  ...
  if (inherits(williams_result, "try-error")) {
    warning(
      "Williams test not conducted, probably due to input being a formular. Error: ",
    ...
    result <- try(
      {
        if (alternative != "two.sided") {
          stop(
            "DescTools package does not accept one-sided test, try other methods instead. We recommend multcomp as the standard approach. "
          )
        }
