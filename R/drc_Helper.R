  ...
  if(!inherits(object,"try-error")){
    if(CI=="delta" | CI=="inv"){
      ...
      if(grepl("log(ED50) as parameter",object$fct$text,fixed = TRUE)) {
        ...
        ##browser()
      }else interval <- CI ## could be "delta" or "inv"
      if("c:(Intercept)" %in% names(coefs)) cVal <- coefs["c:(Intercept)"] else cVal <-0 ## switch(response,Increase=0,Decrease=1)
      if("d:(Intercept)" %in% names(coefs)) d <- coefs["d:(Intercept)"] else d <- 1
...
#'
#' @examples
mselect.plus <- function (object=NULL, fctList = NULL, nested = FALSE, sorted = c("IC",
...
#'
#' @examples
drcCompare <- function(modRes=NULL,modList=NULL,trend = "Decrease",CI=c("delta","inv","bmd-inv"),...){
  CI <- match.arg(CI)
  if(!is.null(modRes)){
