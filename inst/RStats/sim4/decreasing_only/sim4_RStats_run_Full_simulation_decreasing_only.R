setwd("~/Projects/drcHelper/inst/RStats/sim4/decreasing_only")
library(parallel)
library(doParallel)
library(drcHelper)
...
dim(param_grid)
sim4_decreasing_results <- run_multiple_simulations_with_logging(
  param_grid = param_grid,
  test_methods = test_methods,
  n_sim = 1000
)
saveRDS(sim4_decreasing_results,file="sim4_decreasing_results.rds")
...
if(plotit){
  ...
  full_results <- dplyr::left_join(param_grid,sim4_decreasing_results)
  ...
  # }
    ...
    full_results <- dplyr::left_join(param_grid,sim4_decreasing_results) %>% filter(response_type == "decreasing")
    design_effect <- full_results %>% group_by(max_effect,response_type,Dose_Level) %>%
      reframe(neffect=length(unique(Expected_Response)), Reduction = paste0(100-Expected_Response[1],"%"))
    ...
    for(m_tank0 in c(4,6)){
      for(k_individuals0 in c(3,6,10)){
        ...
        ggsave(paste0("SimPower_",m_tank0, "_tank_",k_individuals0,"_ind_",max_effect0,"_effect.pdf"),dpi=300, width = 6,height =6)
        ggsave(paste0("SimPower_",m_tank0, "_tank_",k_individuals0,"_ind_",max_effect0,"_effect.png"),dpi=300, width = 6,height =6)
