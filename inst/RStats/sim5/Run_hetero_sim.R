...
if(plotit){
  ...
  for(max_effect0 in c(5,20)){
    if(max_effect0 ==5){
      tempdata <- sim5_results ##%>% ## dplyr::filter(response_type!="oscillating") %>% droplevels(.)
      design_effect_temp <- design_effect # %>% dplyr::filter(response_type != "oscillating")
    }else{
      tempdata <- sim5_results%>%dplyr::filter(response_type!="none") %>% droplevels(.)
      design_effect_temp <- design_effect %>% dplyr::filter(response_type != "none")
    }
    for(m_tank0 in c(4,6,10)){
      for(base_variance0 in c(4,6)){

        ggplot(tempdata%>%
                 dplyr::filter(m_tanks==m_tank0, base_variance==base_variance0,
                               max_effect==max_effect0),aes(x=Dose_Level,y=Power,color=Method))+
          geom_point(aes(pch=Method))+
          facet_grid(response_type ~ variance_pattern,scales = "free")  +
          geom_line()+
          geom_hline(yintercept = c(0.05,0.8),lty=2,alpha=0.3)+
          theme(legend.position = "bottom")+ ## ggplot2::scale_color_viridis_d(direction = 1)
          ggthemes::scale_color_solarized()+ggtitle(paste0(m_tank0, " tanks ", base_variance0," base variance, maximum effect: ", max_effect0, "%"))+
          geom_text(data=design_effect%>%dplyr::filter(max_effect==max_effect0,base_variance==base_variance0), aes(x=Dose_Level,y=1.1,label=paste(Reduction, "\nVar=",Variance)), size=3,col= "black" )
        #ggsave(paste0("SimPower_",m_tank0, "_tank_",base_variance0,"_base_variance_",max_effect0,"_effect.png"),dpi=300, width = 6,height =5)
    ...
    for(m_tank0 in c(4,6,10)){
      for(response_type0 in c("none","oscillating","threshold")){
        ...
        ggplot(tempdata%>%
          ...
          geom_text(data=design_effect%>%dplyr::filter(max_effect==max_effect0,response_type == "oscillating"), aes(x=Dose_Level,y=1.15,label=paste("Var=",Variance)), size=3,col= "black" )
        ggsave(paste0("SimHetero_",m_tank0, "_tank_","response_type_", response_type0,max_effect0,"_effect.png"),
