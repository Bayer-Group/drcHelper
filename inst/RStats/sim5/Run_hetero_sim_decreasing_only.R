...
param_grid_hetero <- expand.grid(
  ...
  response_type = c("decreasing"),  # Focus on null and decreasing
  alpha = 0.05,
  alternative = "less",
...
sim5_decreasing_results <- run_multiple_hetero_simulations(param_grid=param_grid_hetero,
...
saveRDS(sim5_decreasing_results,file="sim5_decreasing_results.rds")
...
if(plotit){
  ...
  theme_set(theme_bw())
  ##
  sim5_decreasing_results$Method[sim5_decreasing_results$Method=="GLS_Hetero"] <- "LM-Hetero"
  sim5_decreasing_results$Method[sim5_decreasing_results$Method=="LM_Hetero"] <- "LM-Hetero"
  sim5_decreasing_results$Method[sim5_decreasing_results$Method=="LM_Homo"] <- "LM-Homo"
  sim5_decreasing_results$Dose_Level <- gsub("Dose_","T",paste0(sim5_decreasing_results$Dose_Level,"\n",(100-sim5_decreasing_results$Expected_Response),"%"))
  design_effect <- sim5_decreasing_results %>% dplyr::group_by(max_effect,response_type,Dose_Level,variance_pattern,base_variance) %>%
    dplyr::reframe(neffect=length(unique(Expected_Response)), Reduction = paste0(100-Expected_Response[1],"%"),
        ...
        if(max_effect0 == 20 && response_type0 =="none"){}else{
          ggplot(tempdata%>%
            ...
            geom_text(data=design_effect%>%dplyr::filter(max_effect==max_effect0,response_type == response_type0)%>%droplevels(.),
                      aes(x=Dose_Level,y=1.15,label=paste("Var=",Variance)), size=2.5,col= "black" )+xlab("Dose Level")
          ggsave(paste0("inst/RStats/sim5/decreasing_results/SimHetero_",m_tank0, "_tank_","response_", response_type0,max_effect0,"_effect.png"),
                 dpi=300, width = 6,height = 6)
          ggsave(paste0("inst/RStats/sim5/decreasing_results/SimHetero_",m_tank0, "_tank_","response_", response_type0,max_effect0,"_effect.pdf"),
                 dpi=300, width = 6,height = 6)
