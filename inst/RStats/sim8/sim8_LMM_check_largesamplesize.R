...
table(param_grid$k_individuals)
...
saveRDS(full_results,file="sim8_full_results.rds")
...
if(plotit){
  ...
  library(tidyverse)
  theme_set(theme_bw())
  full_results <- dplyr::left_join(param_grid,sim8_full_results) %>% mutate(k_individuals = factor(k_individuals))
  full_results <- dplyr::left_join(sim8_full_results,param_grid) %>% mutate(k_individuals = factor(k_individuals))
        ...
        ggsave("SimPower_LMM_alpha_largeSampleSize.png",dpi=300, width = 6,height =5)
