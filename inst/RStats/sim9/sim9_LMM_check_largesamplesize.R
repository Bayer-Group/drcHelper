rm(list=ls())
setwd("~/Projects/drcHelper/inst/RStats/sim9")
library(parallel)
library(doParallel)
library(drcHelper)
...
icc_values <- c(0.1,0.2, 0.3, 0.5, 0.7, 0.9)  # Low to high ICC
...
saveRDS(full_results,file="sim9_full_results.rds")
...
if(plotit){
  ...
  library(tidyverse)
  theme_set(theme_bw())
  full_results <- dplyr::left_join(param_grid,sim9_full_results) %>% mutate(k_individuals = factor(k_individuals))
        ...
        ggsave("SimPower_LMM_alpha_largeTankReplication.png",dpi=300, width = 6,height =5)
