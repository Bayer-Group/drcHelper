...
#' @param test Character string specifying the test to use: either "cochran_armitage"
#'        or "rao_scott".
#' @param alternative Character string specifying the alternative hypothesis.
#'        Must be one of "two.sided" (default), "greater", or "less".
#' @param rao_scott_phi Overdispersion parameter for Rao-Scott correction. If NULL (default),
...
#' result_rs <- stepDownTrendTestBinom(successes, totals, doses, test = "rao_scott")
#' print(result_rs)
#'
#' @importFrom stats prop.test complete.cases
#' @export
stepDownTrendTestBinom_1 <- function(successes, totals, doses,
                                   test = c("cochran_armitage", "rao_scott"),
  ...
  # Function to perform Cochran-Armitage test
  cochran_armitage_test <- function(succ, tot, dose, alt = "two.sided", rs_phi = NULL) {
    n <- length(succ)
    ...
    mean_score <- sum(tot * scores) / sum(tot)

    # Calculate numerator: sum(tot_i * (score_i - mean_score) * p_i)
    numerator <- sum(tot * (scores - mean_score) * p)

    # Calculate denominator components
    p_overall <- sum(succ) / sum(tot)
    q_overall <- 1 - p_overall

    # Standard denominator for Cochran-Armitage
    denominator <- sqrt(p_overall * q_overall * sum(tot * (scores - mean_score)^2))
    ...
    # Prepare output
    method_name <- if(test == "cochran_armitage") {
    ...
    result <- list(
      ...
      method = method_name,
      alternative = alt,
      data.name = paste("proportions", paste(round(p, 3), collapse = ", "))
    )
  ...
  for (i in 1:(k-1)) {
    ...
    test_result <- cochran_armitage_test(
      successes[idx],
      totals[idx],
      doses[idx],
      alt = alternative,
      rs_phi = rao_scott_phi
    )
  ...
  if (is.na(sig_idx)) {
    ...
    noec <- doses[k - sig_idx + 1]
    loec <- doses[k - sig_idx + 2]
  }
