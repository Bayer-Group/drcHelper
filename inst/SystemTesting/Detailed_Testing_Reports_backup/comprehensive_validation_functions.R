# Complete multi-endpoint Dunnett validation function for the main report
# This replaces the run_dunnett_validation function in the comprehensive report
...
run_dunnett_validation <- function(study_id, function_group_id, alternative = "less") {
  ...
  # Handle case where all_validation_results is NULL or empty
  ...
  return(list(
    ...
    n_comparisons = ifelse(is.null(all_validation_results) || nrow(all_validation_results) == 0, 0, nrow(all_validation_results)),
    n_passed = ifelse(is.null(all_validation_results) || nrow(all_validation_results) == 0, 0, sum(all_validation_results$passed))
  ))
}