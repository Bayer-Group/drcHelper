...
2. **All other studies**: Match on **Study ID + Endpoint only** (ignore measurement variable mismatch)

```{r data_matching_logic, eval=FALSE}
...
**Issue**: Studies can have multiple endpoints with different data types. For example, study "MOCK08/15-001" has:
- **Mortality** endpoint: Count data (Alive/Dead/Total columns)
- **Reproduction** endpoint: Continuous data (numeric response)
- **Repellency** endpoint: Continuous data (percentage response)
...
- **Missing value**: `NA` (when control is not numerically quantifiable)

The test functions must handle both cases appropriately:

```{r control_dose_handling, eval=FALSE}
...
2. **NA Handling**: When control is NA, comparisons should be made relative to the control group, not a numeric dose level
3. **Dose Conversion**: European decimal notation (comma separators) must be converted to standard format before processing
...
2. **Data Matching Logic**: Implement the corrected matching logic where MOCK0065 requires 3-field matching (Study ID + Endpoint + Measurement Variable) while other studies use 2-field matching (Study ID + Endpoint only).
...
3. **Implementation Priority**: Focus on continuous data scenarios (FG00220, FG00225) as these represent the most common use cases.
