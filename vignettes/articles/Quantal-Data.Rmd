---
title: "Quantal Data"
---


Analyis of quantal data typically involves the following steps:

1. Check if both solvent control and negative control present. If yes, check if the two control groups differ using Fisher
s Exact test. 
2. If there are more than 2 dose groups, determin if a monotonic dose-response is expected. 

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  warning = FALSE,
  message = FALSE
)
library(tidyverse)
```

```{r}
library(drcHelper)
```


## Tarone's Test

```{r}
mymatrix1 <- matrix(c(4,5,5,103),nrow=2,byrow=TRUE)
colnames(mymatrix1) <- c("Disease","Control")
rownames(mymatrix1) <- c("Exposure","Unexposed")
mymatrix2 <- matrix(c(10,3,5,43),nrow=2,byrow=TRUE)
colnames(mymatrix2) <- c("Disease","Control")
rownames(mymatrix2) <- c("Exposure","Unexposed")
mylist <- list(mymatrix1,mymatrix2)
calcTaronesTest(mylist)
```

## Trend Test

```{r}

```

